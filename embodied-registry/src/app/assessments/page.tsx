...
export default function AssessmentsPage() {
  return <><SiteHeader /><main className="evidence-page">
    ...
    <section><h2>Published assessments</h2><div className="assessment-list">{externalPolicyAssessments.map(record => <article key={record.slug}><p className="kicker">METADATA ONLY · {record.assessment.status.toUpperCase()}</p><h3><Link href={`/assessments/${record.slug}`}>{record.title}</Link></h3><p>{record.summary}</p><p><strong>Source:</strong> {record.source.repository} · <code>{record.source.revision.slice(0, 12)}</code> · {record.source.license}</p><p>{record.findings.errors.length} missing requirements · {record.findings.warnings.length} cautions · no evaluation performed</p></article>)}</div></section>
