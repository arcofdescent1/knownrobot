import { z } from "zod";
import { EvidenceRecord, displayStatus, statusLabels, safeEvidenceUrl, isSimulation } from "./evidence-contract";
...
export function evidenceBadge(record: EvidenceRecord) {
  ...
  const value = `${isSimulation(record) ? "Simulation · " : ""}${statusLabels[status]} · ${family}`;
...
export function citation(record: EvidenceRecord) {
  return {
    ...
    title: `${record.skill.name}: ${isSimulation(record) ? "simulation evaluation" : "evaluation"} on ${record.hardware.robot_family}`,
